  ...
  const searchMovies = async (endpoint, query) => {
    try {
      const response = await fetch(
        `${TMDB_API_URL}/search/${endpoint}?query=${encodeURIComponent(
          query
...
Only give me names of exactly 5 movies, comma separated.
Example format: Spider Man, Elemental, Phir Hera Pheri
        `.trim();

      // Gemini call (replacing OpenAI)
      const text = await generateText(prompt, "gemini-1.5-flash");

      // Be robust to bullets/newlines; take first 5 names
      const gptResults = text
        .replace(/\*/g, "")
        .split(/[,|\n]/)
        .map((s) => s.trim())
        .filter(Boolean)
      ...
      const data = gptResults.map((q) => searchMovies("movie", q));
      const searchResults = await Promise.all(data);

      if (searchResults) setLoadingBtn(false);
